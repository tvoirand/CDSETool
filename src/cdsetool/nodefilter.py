...
def search_nodes(node_url: str, pattern: str, exclude: bool = False) -> List[Dict[str, Any]]:
    ...
    If "exclude" is set to False, only nodes that match pattern are returned. If it's set to True,
    only nodes that do not match pattern are returned.
    """
...
def download_node(
    ...
    """
    Download a single feature with node filtering to include only some files within the product.
    """
    ...
    for node in nodes:
        ...
        if response.status_code == 200:
            with open(
                os.path.join(path, node["Name"]), "wb"  # TODO: Reproduce directories tree structure
            ) as file:
